...
class Event[ContextType: GameContext, **Params, ReturnType]:
    def __init__(
        ...
        self.has_return_type = signature(func, eval_str=True).return_annotation != None
    ...
    def __call__(self, *args: Params.args, **kwargs: Params.kwargs) -> ReturnType:
        ...
        if self.event.callback and self.do_callback:
            try:
                ...
                gctx.logger.warning("Unable to run callback, no event loop is running!")
        return ret
...
def event[ContextType: GameContext, **Params, ReturnType](*, callback: CallbackWithReturnedVal[ContextType, ReturnType] | CallbackWithoutReturnedVal[ContextType] | None = None):
    def inner(func: Callable[Concatenate[ContextType, Params], ReturnType], /) -> Event[ContextType, Params, ReturnType]:
        ...
        if sig.return_annotation == Signature.empty:
            raise ValueError(f"Invalid function {func} marked as event: return type must be annotated!")

        for name, param in sig.parameters.items():
            if name == "self": continue

            if param.annotation == Parameter.empty:
                raise ValueError(f"Invalid function {func} marked as event: parameter {name} type must be annotated!")
